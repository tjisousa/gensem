...
### Fixed
- `install.py confirm()` crashed with `EOFError` under `curl | sh` because stdin is piped (not a TTY) — the no-plugin "Continue?" prompt blew up the whole install. `confirm()` now auto-accepts when `sys.stdin.isatty()` is false, preserving install.sh's zero-prompt contract. Interactive TTY behavior is unchanged.
...
### Added
- `GSE_OWNER_REPO` env-var override in `install.sh` for testing against forks (e.g. `GSE_OWNER_REPO=tjisousa/gensem`). Default unchanged (`nicolasguelfi/gensem`).
