...
3. **Lifecycle guardrails:**
   ...
   | Starting REVIEW without test evidence (TASK `test_evidence` is missing OR `test_evidence.status` is `absent` or `skipped`) | **Hard** | Block. Run tests first. "Tests must pass before review." A `status: fail` does NOT block review — it triggers a Soft guardrail that surfaces the failures as RVW- findings with severity HIGH. |
   | Starting PRODUCE with unresolved `[STRATEGY]` or `[TST-SPEC]` HIGH findings in the current sprint's review.md (spec §6.5) | **Hard** | Block. Resolve the strategy / TST-spec findings first. "Tests reviewed upstream must pass before code is written against them." MEDIUM / LOW findings in these tiers warn but do not block. |
