...
**Pedagogical evaluation at activity start (tutor agent):** at the beginning of any activity (`/gse:collect`, `/gse:assess`, `/gse:plan`, `/gse:reqs`, `/gse:design`, `/gse:preview`, `/gse:tests`, `/gse:produce`, `/gse:review`, `/gse:fix`, `/gse:deliver`, `/gse:compound`, `/gse:integrate`, `/gse:task`, `/gse:deploy`), if `profile.yaml → dimensions.learning_goals` is non-empty AND `config.yaml → pedagogy.enabled: true` AND the sprint's preamble cap (`pedagogy.max_preambles_per_sprint`, default 3) is not exhausted, the orchestrator spawns the **`tutor`** sub-agent for a **contextual pedagogical evaluation** (see `agents/tutor.md` for the full algorithm). The tutor returns either *"skip"* (silent) or *"propose"* with a precise topic formulation and 5-option P14 preamble content. On *"propose"*, the orchestrator presents the standard P14 Gate (Quick overview / Deep session / Not now / Not interested / Discuss) to the user; the response is persisted in `status.yaml → learning_preambles[]` to respect subsequent choices. The tutor also handles **inferred gap detection** (repeated questions, hesitations, explicit confusion) when `pedagogy.proactive_gap_detection: true`. Rationale: a sub-agent with fresh context keeps the pedagogical reasoning objective and separated from project-execution concerns, while a lightweight invocation contract avoids cluttering the main conversation.
